...
<form action="logic/functions.php" method="post" class="card-body" style="width: 300px; border: 1px solid black;">
  ADD SELECTION
  <input type="text" name="selection_name"  class="form-control">
  <input type="hidden"  name="product_id" value="<?php echo $_GET['item-id']; ?>" required>
  
<button type="submit" class="btn btn-primary" name="add_selection_name">ADD</button>
...
<form action="logic/functions.php" method="post"  class="card-body" style="width: 300px; border: 1px solid black;">
SELECTION
 <select name="selection_id" class="form-control">
  <?php 
...
ADD OPTIONS
  <input type="text" name="option_name" value="" class="form-control" required>

  <input type="hidden" name="product_id" value="<?php echo $_GET['item-id']; ?>" required>
  
<button type="submit" name="add_option_name" class="btn btn-primary">ADD</button>
...
<form action="logic/functions.php" method="post">
    ...
    <?php 
  $id = $_GET['item-id'];
$qryView = mysqli_query($db, "SELECT is_available,item_id,item_name,item_img,item_price,gcash_qrcode,total_items_left FROM items WHERE item_id='$id' ");


  if($row=mysqli_fetch_array($qryView)):

    $_SESSION['prc'] = $row['item_price'];
    $stock = $row['total_items_left'];
        ...
        <div class="product_img-box text-center" style="background: #F9D6F6;">  
         ...
         <!--  <label>Product Image</label> -->
          <div class="d-flex justify-content-center" style=" height: 340px!important;">
              <img loading=lazy style="background:#F9D6F6;" id="item_img_" src="<?php echo $row['item_img'] ?>" alt="" style=" height: 250px!important;"/>
          </div>
            </div>
...
<div style="margin:5px; border: 0px solid black; width: 100%;" >
 <?php 
      ...
      $selection_name = $row_['selection_name'];
       ...
       $qry_option = mysqli_query($db, "SELECT * FROM options WHERE selection_id='$id' ");
        ...
        while($row_option = mysqli_fetch_assoc($qry_option)):
           ...
           $selection_name_new =   str_replace(" ","",$selection_name);

           echo '  <input type="radio"
            id="'.$row_option['option_id'].'" 
           name="'.$selection_name_new.'" 
           class="selector-item_radio" 
           value="'.$row_option['option_id'].'" required>
       ...
       echo '<input type="hidden" name="selection_names[]" value="'.$selection_name_new.'" >';
...
<div>
      ...
      <input type="hidden" name="item_id" value="<?php echo $_GET['item-id']; ?>" >


    <div class="detail-info" style="width:100%">
      <div class=" text-white">
         <p id="final_price" style=" background: none; color: white;font-size: 15px" class="info badge bg-success"> ₱<?php echo number_format((float)$row['item_price'], 2, '.', ',');  ?></p> 
        <input type="hidden" id="final_price_input" value="<?php echo $row['item_price']; ?>" >
          <p class="badge bg-success text-white" style="font-size: 15px">Stock : <?php echo $stock; ?></p>
      </div>
      


      <div class="input-field">
        <label for="card_number " style="font-size: 15px" class="badge bg-success text-white">Quantity
        <input onkeypress="validate(event)" value="" type="number" class="form-control" minlength="1" min="1" maxlength="3" id="quantity" name="quantity" placeholder="number of products" required></label>
      </div>
...
<!-- 
    ...
    <div id="smart-button-container" style="margin" class="mt-3 row">
...
<button type="submit" name="add-cart" class="btn btn-primary" style="width:100%"> <img src="images/add-cart.png" style="width: 24px; height: 25px;"> Add to Cart</button>

<?php } else { ?>

<div class="btn btn-danger" style="width:100%; hover:none">Item Unvailable</div>
...
<?php include('footer.php'); ?>
  ...
  <script>
      ...
      //EVENTS
        ...
        document.getElementById("quantity").addEventListener("click", changePrice);
...
console.log(p1);
    ...
    // $('input[class=selector-item_radio]').on('change', function (e) {
    // if ($('input[class=selector-item_radio]:checked').length > <?php //echo $limit_box; ?>) {
